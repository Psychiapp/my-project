-- Migration: Fix get_all_eligible_supporters function overload
-- Drops the old single-parameter version to avoid ambiguity with the two-parameter version

-- Drop the old single-parameter version (if exists)
DROP FUNCTION IF EXISTS get_all_eligible_supporters(TEXT);

-- Recreate the function with proper default values
-- This is the authoritative version with schedule conflict checking
